...
4. 基于Metal的神经渲染、RealityKit里的布料仿真与3DGS、RCP3的数字人相关组件、visionOS的全景图转3D以及新版地图效果与我强相关，后续会逐个研究下细节。
...
### 思考
没看到太多独占性的新功能，除了Foveated Streaming和沉浸式音频，其他大都为现有功能的例行升级，大概率是因为这块的研发速度因为业务调整（渠道反馈 Apple Vision Pro 停产，相关研发计划暂停）有点放慢了。
...
2. 对比看Reality Composer Pro 3，与我对普适的 3D 互动的思考是一致的，基本覆盖了团队这几年花了大量时间建设的 NavMesh、BT、Shader Graph、Script Graph 、VFX Graph能力，当前也在构建 Anim Graph。
3. 比较可惜的是，当前团队的自研编辑器是暂停的状态，这块当前还是深度和 Unity 编辑器绑定，好处是 Unity 编辑器好用生态成熟，坏处是部分环节开发工作量翻倍，且受限于 Unity 的既有框架限制部分能力难以实现（如 GPU Driven 3DGS Render Pipeline）。
...
4. 对比RealityKit，团队自研引擎在图形层面当前没有高级软阴影/布料仿真/Projective Textures，其他的至少不弱于 RealityKit，现在缺失的能力后面也会慎重评估一下重要程度看是否需要补上。整体来看，自研引擎相比于RealityKit的优势在缩小，但跨平台支持上的优势应该会持续存在。
5. RealityKit的PBR Shading Model对于金属表达比Unity明显好，主要反映在高光建模，这点前面做了一些分析，后面结合新的3d模型需求进一步完善，把现有的渲染效果做提升，这个还挺有意思的。
